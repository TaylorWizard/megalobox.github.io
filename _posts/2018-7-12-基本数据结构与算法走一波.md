...
- ##### 复杂度分析
  - 最好情况
    $$ O(n\log_2n) $$
    > 为了方便计算大O表示法的答案，我们让每次分割都是均等的就有，令T(1) = 0, 因为一个数不需要排序, 那么有下面的式子:
    <span style="display:block;text-align:center;">$$ T(N) = 2T(N/2) + cN $$</span>
    <span style="display:block;text-align:center;">$$T(N) = 2(2T(\dfrac{N}{4})+ \dfrac{N}{2}) + cN $$</span>
    <span style="display:block;text-align:center;">$$T(N) = 4(2T(\dfrac{N}{8})+ \dfrac{N}{4}) + 2cN $$</span>
    <span style="display:block;text-align:center;">$$T(N) = 8(2T(\dfrac{N}{16})+ \dfrac{N}{8}) + 3cN $$</span>
  ...
  ```javascript
    class MergeSort {
        constructor(array) {
            ...
            MergeSort.mergeSortHelper(this.array, this.copybuffer, 0, this.len - 1);
        }

        static mergeSortHelper(array, copybuffer, low, high) {
            let middle = ~~((low + high) / 2);

            if (low < high) {
                MergeSort.mergeSortHelper(array, copybuffer, low, middle);
                MergeSort.mergeSortHelper(array, copybuffer, middle + 1, high);
                MergeSort.merge(array, copybuffer, low, middle, high);
            }
        }

        static merge(array, copybuffer, low, middle, high) {
