...
- 复杂度$$O(n\log_2n)$$分析
  ...
  - 归并排序所花费的时间来自于每次递归和合并的时间的开销, 每次分割成相等的两部分, 并进行两次递归调用, 合并所花的时间是常数级的(只需要一层循环),可以得到以下公式:
    <span style='display:block;text-align:center;'>$$T(N) = 2T(\frac{N}{2}) + cN$$</span>
    <span style='display:block;text-align:center;'>$$T(N) = 2(2T(\frac{N}{4}) + \frac{cN}{2}) + cN$$</span>
    <span style='display:block;text-align:center;'>$$T(N) = 4(2T(\frac{N}{8}) + \frac{cN}{4}) + 2cN$$</span>
