...
- ##### 复杂度分析
  - 最好情况
    $$ O(n\log_2n) $$
    > 为了方便计算大O表示法的答案，我们让每次分割都是均等的就有，那么有下面的式子:
    <div>$$ T(N) = 2T(N/2) + cN $$</div> 
    <div>$$T(N) = 2(2T(\dfrac{N}{4})+ \dfrac{N}{2}) + N $$</div>
  - 最坏情况
  $$ O(n^2) $$
    ...
    <div>$$ T(N) = T(N - i) + T(N - i - 1) + cN $$</div>
    > - i为枢纽元一侧的元素个数。
    > - 由于是最坏的情况, 枢纽元始终是第一个元素或者最后一个元素,那么i等于0，忽略T(0) = 1
    那么有如下关系 
    <div>$$T(N) = T(N - 1) + cN $$</div>
    <div>$$ T(N - 1) = T(N - 2) + c(N - 1)$$</div> 
    <div>$$ T(N - 2) = T(N - 3) + c(N - 2)$$</div> 
    <div>$$....$$</div> 
    <div>$$T(2) = T(1) + c(2)$$</div> 
    <div>把如上所有式子相加:</div> 
    <div>$$T(N) = T(1) + c\sum_{j=2}^Nj = c\left(\frac{(2+n)n}{2}\right) = O(n^2)$$</div>
