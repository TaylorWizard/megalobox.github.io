...
- 复杂度$$O(n\log_2n)$$分析
  ...
  - 归并排序所花费的时间来自于每次递归和合并的时间的开销, 每次分割成相等的两部分, 并进行两次递归调用, 合并所花的时间是常数级的(只需要一层循环),可以得到以下公式:
    <span style='display:block;text-align:center;'>$$T(N) = 2T(\frac{cN}{2}) + cN$$</span>
    <span style='display:block;text-align:center;'>$$T(N) = 2(2T(\frac{cN}{4}) + \frac{cN}{2}) + cN$$</span>
    <span style='display:block;text-align:center;'>$$T(N) = 4(2T(\frac{cN}{8}) + \frac{cN}{4}) + 2cN$$</span>
    <span style="display:block;text-align:center;">$$T(N) = 8(2T(\dfrac{cN}{16})+ \dfrac{cN}{8}) + 3cN $$</span>
    <span style="display:block;text-align:center;">$$ ... $$</span>
    <span style="display:block;text-align:center;">$$T(N) = N^{\log_2N}T(1) + cN(\log_2N) = O(N\log_2N)$$</span>
    - tips: 上面式子经过了$$log_2N$$次计算,从式子我们可以看到最后$$\dfrac{N}{2^k} = 1$$, 可以推出 $$k = \log_2N$$
